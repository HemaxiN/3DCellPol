...
def cellpol_data_creation(image_names, gt_dir, images_dir, nucleus_radius, golgi_radius, size_, step_, save_imgs_dir, save_outputs_dir):
    ...
    for image_nb in numbers:
        ...
        ## zero padding
        aux_img = np.random.randint(1,10,(aux_sizes[0], aux_sizes[1], 64,2)).astype('uint8')
        aux_img[0:aux_sizes_or[0], 0:aux_sizes_or[1],:np.shape(image)[2],:] = image[:,:,:,0:2]
        image = aux_img
        ...
        for row in reader:
            if row[0] != 'YN,XN,ZN,YG,XG,ZG':
                ...
                XG = float(aux[3])
                ZG = float(aux[5])
                centroids_nuclei.append((XN,YN,ZN))
                centroids_golgi.append((XG,YG,ZG))
        ...
        while i+size_<=image.shape[0]:
            ...
            while j+size_<=image.shape[1]:
                ...
                for cn, cg in zip(centroids_nuclei, centroids_golgi):
                   cn2 = [cn[0]-i,cn[1]-j, cn[2]]
                   cg2 = [cg[0]-i,cg[1]-j, cg[2]]
                   if(0<=cn2[0]<size_ and 0<=cg2[0]<size_ and 0<=cn2[1]<size_ and 0<=cg2[1]<size_):
                       cnn.append([int(cn2[0]), int(cn2[1]), cn2[2]])
                       cgg.append([int(cg2[0]), int(cg2[1]), cg2[2]])
                       vecs_.append([int(cn2[1]), int(cn2[0]), cn2[2], int(cg2[1]), int(cg2[0]), cg2[2]])
                ...
                if len(cnn)>1:
                    ...
                    centroids_image = np.zeros((np.shape(_slice)[0], np.shape(_slice)[1], 64, 4))
                    centroids_image[:,:,:,1], centroids_image[:,:,:,3] = centroids2Images(cnn, np.shape(_slice)[0], np.shape(_slice)[1], 64, g_radius=nucleus_radius, th=0.5)
                    centroids_image[:,:,:,0], centroids_image[:,:,:,2] = centroids2Images(cgg, np.shape(_slice)[0], np.shape(_slice)[1], 64, g_radius=golgi_radius, th=0.5)
