...
from keras.layers import Input, Conv2D, MaxPooling2D, UpSampling2D, Activation, BatchNormalization, Dropout, Conv2DTranspose
...
def weighted_mean_se(y_true, y_pred):
    [golgi_c_gt, nuclei_c_gt, golgi_weight, nuclei_weight, _, _] = tf.unstack(y_true, 6, axis=3)
    [golgi_c_pred, nuclei_c_pred, _, _] = tf.unstack(y_pred, 4, axis=3)
...
def pull_aux_optimized(y, mask_g, embeddings_g, mask_n, embeddings_n):
    ...
    pull = 0.
    E_tensor = []
    for filter_value in range(0, 200):
        if filter_value != 0:
            ...
            paux = tf.reduce_mean(tf.math.pow((filtered_values-mean_),2))
...
def pull_loss(y_true, y_pred, bi):
    [gctr, nctr, golgi_embeddings, nuclei_embeddings] = tf.unstack(y_pred, 4, axis=3)
    [gctr_gt, nctr_gt, wg, wn, golgi_mask_gt, nuclei_mask_gt] = tf.unstack(y_true, 6, axis=3)
...
def threeDCellPol(n_classes=3, im_sz=256, n_channels=2, n_filters_start=32, growth_factor=2, upconv=True):
        droprate=0.10
        n_filters = n_filters_start
        inputs = Input((im_sz, im_sz, n_channels))
        #inputs = BatchNormalization(axis=-1)(inputs)
        conv1 = Conv2D(n_filters, (3,3), activation='relu', padding='same', data_format='channels_last')(inputs)
        conv1 = Conv2D(n_filters, (3,3), activation='relu', padding='same', data_format='channels_last')(conv1)
        pool1 = MaxPooling2D(pool_size=(2,2), data_format='channels_last')(conv1)
        #pool1 = Dropout(droprate)(pool1)

        n_filters *= growth_factor
        pool1 = BatchNormalization(axis=-1)(pool1)
        conv2 = Conv2D(n_filters, (3,3), activation='relu', padding='same', data_format='channels_last')(pool1)
        conv2 = Conv2D(n_filters, (3,3), activation='relu', padding='same', data_format='channels_last')(conv2)
        pool2 = MaxPooling2D(pool_size=(2,2), data_format='channels_last')(conv2)
        pool2 = Dropout(droprate)(pool2)

        n_filters *= growth_factor
        pool2 = BatchNormalization(axis=-1)(pool2)
        conv3 = Conv2D(n_filters, (3,3), activation='relu', padding='same', data_format='channels_last')(pool2)
        conv3 = Conv2D(n_filters, (3,3), activation='relu', padding='same', data_format='channels_last')(conv3)
        pool3 = MaxPooling2D(pool_size=(2,2), data_format='channels_last')(conv3)
        pool3 = Dropout(droprate)(pool3)

        n_filters *= growth_factor
        pool3 = BatchNormalization(axis=-1)(pool3)
        conv4_0 = Conv2D(n_filters, (3,3), activation='relu', padding='same', data_format='channels_last')(pool3)
        conv4_0 = Conv2D(n_filters, (3,3), activation='relu', padding='same', data_format='channels_last')(conv4_0)
        pool4_1 = MaxPooling2D(pool_size=(2,2), data_format='channels_last')(conv4_0)
        pool4_1 = Dropout(droprate)(pool4_1)

        n_filters *= growth_factor
        pool4_1 = BatchNormalization(axis=-1)(pool4_1)
        conv4_1 = Conv2D(n_filters, (3,3), activation='relu', padding='same', data_format='channels_last')(pool4_1)
        conv4_1 = Conv2D(n_filters, (3,3), activation='relu', padding='same', data_format='channels_last')(conv4_1)
        pool4_2 = MaxPooling2D(pool_size=(2,2), data_format='channels_last')(conv4_1)
        pool4_2 = Dropout(droprate)(pool4_2)

        n_filters *= growth_factor
        conv5 = Conv2D(n_filters, (3,3), activation='relu', padding='same', data_format='channels_last')(pool4_2)
        conv5 = Conv2D(n_filters, (3,3), activation='relu', padding='same', data_format='channels_last')(conv5)

        n_filters //= growth_factor
        if upconv:
                up6_1 = concatenate([Conv2DTranspose(n_filters, (2,2), strides=(2,2), padding='same', data_format='channels_last')(conv5), conv4_1])
        else:
                up6_1 = concatenate([UpSampling2D(size=(2,2))(conv5), conv4_1])
        up6_1 = BatchNormalization(axis=-1)(up6_1)
        conv6_1 = Conv2D(n_filters, (3,3), activation='relu', padding='same', data_format='channels_last')(up6_1)
        conv6_1 = Conv2D(n_filters, (3,3), activation='relu', padding='same', data_format='channels_last')(conv6_1)
        conv6_1 = Dropout(droprate)(conv6_1)

        n_filters //= growth_factor
        if upconv:
                up6_2 = concatenate([Conv2DTranspose(n_filters, (2,2), strides=(2,2), padding='same', data_format='channels_last')(conv6_1), conv4_0])
        else:
                up6_2 = concatenate([UpSampling2D(size=(2,2))(conv6_1), conv4_0])
        up6_2 = BatchNormalization(axis=-1)(up6_2)
        conv6_2 = Conv2D(n_filters, (3,3), activation='relu', padding='same', data_format='channels_last')(up6_2)
        conv6_2 = Conv2D(n_filters, (3,3), activation='relu', padding='same', data_format='channels_last')(conv6_2)
        conv6_2 = Dropout(droprate)(conv6_2)

        n_filters //= growth_factor
        if upconv:
                up7 = concatenate([Conv2DTranspose(n_filters, (2,2), strides=(2,2), padding='same', data_format='channels_last')(conv6_2), conv3])
        else:
                up7 = concatenate([UpSampling2D(size=(2,2))(conv6_2), conv3])
        up7 = BatchNormalization(axis=-1)(up7)
        conv7 = Conv2D(n_filters, (3,3), activation='relu', padding='same', data_format='channels_last')(up7)
        conv7 = Conv2D(n_filters, (3,3), activation='relu', padding='same', data_format='channels_last')(conv7)
        conv7 = Dropout(droprate)(conv7)


        n_filters //= growth_factor
        if upconv:
                up8 = concatenate([Conv2DTranspose(n_filters, (2, 2), strides=(2, 2), padding='same', data_format='channels_last')(conv7), conv2])
        else:
                up8 = concatenate([UpSampling2D(size=(2, 2))(conv7), conv2])
        up8 = BatchNormalization(axis=-1)(up8)
        conv8 = Conv2D(n_filters, (3, 3), activation='relu', padding='same', data_format='channels_last')(up8)
        conv8 = Conv2D(n_filters, (3, 3), activation='relu', padding='same', data_format='channels_last')(conv8)
        conv8 = Dropout(droprate)(conv8)


        n_filters //= growth_factor
        if upconv:
                up9 = concatenate([Conv2DTranspose(n_filters, (2, 2), strides=(2, 2), padding='same', data_format='channels_last')(conv8), conv1])
        else:
                up9 = concatenate([UpSampling2D(size=(2, 2))(conv8), conv1])
        conv9 = Conv2D(n_filters, (3,3), activation='relu', padding='same', data_format='channels_last')(up9)
        conv9 = Conv2D(n_filters, (3,3), activation='relu', padding='same', data_format='channels_last')(conv9)

        conv9_n = Conv2D(n_filters, (3,3), activation='relu', padding='same', data_format='channels_last')(conv9)
        conv9_n = BatchNormalization(axis=-1)(conv9_n)

        conv9_g = Conv2D(n_filters, (3,3), activation='relu', padding='same', data_format='channels_last')(conv9)
        conv9_g = BatchNormalization(axis=-1)(conv9_g)

        #nuclei branch
        convnuclei_centroids = Conv2D(n_filters, (3,3), activation='relu',  padding='same', data_format='channels_last')(conv9_n)
        convnuclei_embeddings = Conv2D(n_filters, (3,3), activation='relu',  padding='same', data_format='channels_last')(conv9_n)
        convnuclei_centroids = Conv2D(1, (1, 1), activation='sigmoid', data_format='channels_last')(convnuclei_centroids)
        convnuclei_embeddings = Conv2D(1, (1, 1), activation='linear', data_format='channels_last')(convnuclei_embeddings)

        #golgi branch
        convgolgi_centroids = Conv2D(n_filters, (3,3), activation='relu',  padding='same', data_format='channels_last')(conv9_g)
        convgolgi_embeddings = Conv2D(n_filters, (3,3), activation='relu',  padding='same', data_format='channels_last')(conv9_g)
        convgolgi_centroids = Conv2D(1, (1, 1), activation='sigmoid', data_format='channels_last')(convgolgi_centroids)
        convgolgi_embeddings = Conv2D(1, (1, 1), activation='linear', data_format='channels_last')(convgolgi_embeddings)

        model = Model(inputs=inputs, outputs=concatenate([convgolgi_centroids, convnuclei_centroids, convgolgi_embeddings, convnuclei_embeddings], axis=3))
   
        model.compile(optimizer = opt, loss = both_joint_loss_function, metrics=[weighted_mean_se, pull_loss_vis, push_loss_vis], run_eagerly=False)
        return model
...
class DataGenerator(keras.utils.all_utils.Sequence):
        ...
        def compute_weights(self,X,mask):
            mask_out = np.zeros((np.shape(mask)[0], np.shape(mask)[1], np.shape(mask)[2], 6))
            for i in range(0, np.shape(mask)[0]): #batch size
                aux_x = np.zeros((np.shape(mask)[1], np.shape(mask)[2]))
                aux_y = np.zeros((np.shape(mask)[1], np.shape(mask)[2]))
                mask_out[i,:,:,0] = mask[i,:,:,0]/255.0 #golgi embeddings
                mask_out[i,:,:,1] = mask[i,:,:,1]/255.0 #nuclei embeddings              
                mask_out[i,:,:,2] = 5 + (250*(mask[i,:,:,0]/255.0)) #golgi weights
                mask_out[i,:,:,3] = 5 + (85*(mask[i,:,:,1]/255.0)) #nuclei weights
        ...
        def rotation(self, image, mask, angle):
            ...
            rot_image = cv2.warpAffine(image, matrix, (256,256))
            rot_mask = cv2.warpAffine(mask, matrix, (256,256))
