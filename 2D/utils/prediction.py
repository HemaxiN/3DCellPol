    ...
    while len(nctr_remaining) > 0 and len(gctr_remaining) > 0:
        ...
        for ni, n_value in enumerate(nemb_remaining):
            for gi, g_value in enumerate(gemb_remaining):
                ...
                if distance < min_distance and distance <2 and distance_um(gctr_remaining[gi], nctr_remaining[ni], x_spacing, y_spacing)<35:
...
def prediction(model_dir, images_dir, golgi_dist, nuclei_dist, golgi_prob, nuclei_prob, _patch_size,  _step, save_dir, x_spacing, y_spacing):
    ...
    for img_name in os.listdir(images_dir):
        ...
        print('x_spacing: {}, y_spacing: {}'.format(x_spacing, y_spacing))
        ...
        ## zero padding
        aux_img = np.random.randint(0,10,(aux_sizes[0], aux_sizes[1], 2)).astype('uint8')
        ...
        while ii+_patch_size<=image.shape[0]:
            ...
            while jj+_patch_size<=image.shape[1]:
                ...
                if np.all(_slice==0): #background slices
                    print('background slice')
                else:
                    _slice = _slice/255.0
                    ...
                    golgi_probs = aa_im[xy_golgi[:, 0], xy_golgi[:, 1]]

                    aa_im = output_final[:,:,1].copy()
                    nuclei_probs = aa_im[xy_nuclei[:, 0], xy_nuclei[:, 1]]
                    ...
                    for nctr in xy_nuclei:
                      nemb.append(output_final[nctr[0],nctr[1],3])

                    gemb = []
                    for gctr in xy_golgi:
                      gemb.append(output_final[gctr[0],gctr[1],2])


                    n_centroids_aux, g_centroids_aux, n_probs_new, g_probs_new, n_centroids_new, g_centroids_new = decoding_pairs(xy_nuclei, xy_golgi, nemb, gemb, nuclei_probs, golgi_probs, ii, jj, x_spacing, y_spacing)
        ...
        image_aux = np.zeros((np.shape(image)[0], np.shape(image)[1], 3)).astype('uint8')
        image_aux[:,:,:2] = image
        image = image_aux
        ...
        n_centroids_final, g_centroids_final, new_n_probs, new_g_probs = nms_vectors(n_centroids, g_centroids, n_probs_final, g_probs_final, nuclei_dist, x_spacing, y_spacing) #min nuclei dist
        g_centroids_final, n_centroids_final, _, _ = nms_vectors(g_centroids_final, n_centroids_final, new_g_probs, new_n_probs, golgi_dist, x_spacing, y_spacing)
